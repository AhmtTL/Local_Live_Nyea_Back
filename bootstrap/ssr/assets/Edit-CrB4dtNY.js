...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const form = useForm({
      resource_program_id: props.session.resource_program_id,
    ...
    const submit = () => {
      form.put(route("admin.resource-sessions.update", props.session.id));
    };
    const selectedResourceProgram = computed(() => {
      return props.resourcePrograms.find((p) => p.id === parseInt(form.resource_program_id));
    });
    const togglePublish = () => {
      ...
      ssrRenderVNode(_push, createVNode(resolveDynamicComponent(unref(Layout)), null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</div></div></div><div class="flex gap-3 pt-6"${_scopeId}><button type="button" class="${ssrRenderClass([
              props.session.is_active ? "bg-yellow-600 hover:bg-yellow-700" : "bg-green-600 hover:bg-green-700",
              "px-6 py-2 text-sm font-medium text-white rounded-lg transition-colors"
            ])}"${_scopeId}>${ssrInterpolate(props.session.is_active ? "📝 Send to Draft" : "✅ Publish")}</button><button type="submit"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} class="px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-medium rounded-lg shadow-sm hover:shadow-md transition-all duration-200 transform hover:-translate-y-0.5 disabled:opacity-50"${_scopeId}>${ssrInterpolate(unref(form).processing ? "Saving..." : "Save Changes")}</button>`);
                  ...
                  createVNode("div", { class: "bg-white overflow-hidden shadow-sm sm:rounded-lg" }, [
                    createVNode("div", { class: "p-8 bg-white border-b border-gray-200" }, [
                      createVNode("div", { class: "mb-8" }, [
                        createVNode("h2", { class: "text-3xl font-bold text-gray-900" }, "Edit Resource Session"),
                        createVNode("p", { class: "text-gray-600 mt-2" }, "Update the details of this resource session")
                      ]),
                      createVNode("form", {
                        ...
                        createVNode("div", { class: "flex gap-3 pt-6" }, [
                          ...
                          createVNode("button", {
                            ...
                            class: "px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-medium rounded-lg shadow-sm hover:shadow-md transition-all duration-200 transform hover:-translate-y-0.5 disabled:opacity-50"
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/ResourceSessions/Edit.vue");
