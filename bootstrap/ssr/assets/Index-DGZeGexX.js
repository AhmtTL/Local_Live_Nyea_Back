...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const deleteSession = (sessionId) => {
      if (confirm("Are you sure you want to delete this resource session?")) {
        router.delete(route("admin.resource-sessions.destroy", sessionId));
      }
    };
    const duplicateSession = (sessionId) => {
      if (confirm(
        "Are you sure you want to duplicate this resource session? A copy will be created with updated details."
      )) {
        router.post(route("admin.resource-sessions.duplicate", sessionId));
      }
    };
    const togglePublish = (sessionId, currentStatus) => {
      ...
      if (confirm(
        ...
        router.post(
          route("admin.resource-sessions.toggle-publish", sessionId)
        );
      }
    };
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[-->`);
      _push(ssrRenderComponent(unref(Head), { title: "Resource Sessions Management" }, null, _parent));
      ssrRenderVNode(_push, createVNode(resolveDynamicComponent(unref(Layout)), null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          var _a, _b;
          if (_push2) {
            _push2(`<div class="py-12"${_scopeId}><div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8"${_scopeId}><div class="bg-white overflow-hidden shadow-sm sm:rounded-lg"${_scopeId}><div class="p-6 bg-white border-b border-gray-200"${_scopeId}><div class="flex items-center justify-between mb-6"${_scopeId}><div${_scopeId}><h2 class="text-2xl font-bold text-gray-900"${_scopeId}> Resource Sessions </h2><p class="text-gray-600 mt-1"${_scopeId}> Manage resource sessions across multiple locations </p></div>`);
            _push2(ssrRenderComponent(unref(Link), {
              href: _ctx.route("admin.resource-sessions.create"),
            ...
            _push2(`</div><div class="mb-6"${_scopeId}><input${ssrRenderAttr("value", unref(filters).search)} type="text" placeholder="Search by location, city, country, or resource session name..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"${_scopeId}></div><div class="overflow-x-auto"${_scopeId}><table class="min-w-full divide-y divide-gray-200"${_scopeId}><thead class="bg-gray-50"${_scopeId}><tr${_scopeId}><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>Resource Program</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>School</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>Location</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>Availability</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>Status</th><th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}>Actions</th></tr></thead><tbody class="bg-white divide-y divide-gray-200"${_scopeId}><!--[-->`);
            ssrRenderList(__props.sessions.data, (session) => {
              _push2(`<tr class="hover:bg-gray-50"${_scopeId}>`);
              _push2(ssrRenderComponent(unref(Link), {
                href: _ctx.route("admin.resource-sessions.show", session.id)
              }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  var _a2, _b2, _c, _d;
                  if (_push3) {
                    _push3(`<td class="px-6 py-4"${_scopeId2}><div class="text-sm font-medium text-gray-900"${_scopeId2}>📚 ${ssrInterpolate((_a2 = session.resource_program) == null ? void 0 : _a2.title)}</div><div class="text-sm text-gray-500"${_scopeId2}>${ssrInterpolate((_b2 = session.resource_program) == null ? void 0 : _b2.slug)}</div><div class="text-xs text-gray-600 mt-1"${_scopeId2}>📅 ${ssrInterpolate(formatDateRange(session))}</div></td>`);
                  } else {
                    return [
                      createVNode("td", { class: "px-6 py-4" }, [
                        createVNode("div", { class: "text-sm font-medium text-gray-900" }, "📚 " + toDisplayString((_c = session.resource_program) == null ? void 0 : _c.title), 1),
                        createVNode("div", { class: "text-sm text-gray-500" }, toDisplayString((_d = session.resource_program) == null ? void 0 : _d.slug), 1),
                        createVNode("div", { class: "text-xs text-gray-600 mt-1" }, "📅 " + toDisplayString(formatDateRange(session)), 1)
                      ])
                    ];
                  }
                }),
              ...
              _push2(`</td><td class="px-6 py-4"${_scopeId}><div class="text-sm font-medium text-gray-900"${_scopeId}>⛰️ ${ssrInterpolate(session.location)}</div>`);
              ...
              _push2(`</td><td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"${_scopeId}><div class="flex items-center justify-end space-x-2"${_scopeId}><button class="${ssrRenderClass([session.is_active ? "text-orange-600 hover:text-orange-800 bg-orange-50 hover:bg-orange-100" : "text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100", "px-3 py-1 rounded transition-colors text-sm font-medium"])}"${ssrRenderAttr("title", session.is_active ? "Send to draft" : "Publish session")}${_scopeId}>${ssrInterpolate(session.is_active ? "📝 Send to Draft" : "📢 Publish")}</button>`);
              _push2(ssrRenderComponent(unref(Link), {
                href: _ctx.route("admin.resource-sessions.show", session.id),
              ...
              _push2(ssrRenderComponent(unref(Link), {
                href: _ctx.route("admin.resource-sessions.edit", session.id),
                class: "text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded transition-colors text-sm"
              }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
              ...
              _push2(`<button class="text-orange-600 hover:text-orange-800 bg-orange-50 hover:bg-orange-100 px-3 py-1 rounded transition-colors text-sm" title="Duplicate this session"${_scopeId}>Duplicate</button><button class="text-red-600 hover:text-red-800 bg-gray-100 hover:bg-red-50 px-3 py-1 rounded transition-colors text-sm"${_scopeId}>Delete</button></div></td></tr>`);
            });
            _push2(`<!--]--></tbody></table></div>`);
            if (!((_a = __props.sessions.data) == null ? void 0 : _a.length)) {
              _push2(`<div class="text-center py-12"${_scopeId}><div class="text-gray-400 text-6xl mb-4"${_scopeId}>📚</div><h3 class="text-lg font-medium text-gray-900 mb-2"${_scopeId}>No resource sessions found</h3><p class="text-gray-500 mb-6"${_scopeId}>Get started by creating your first resource session.</p>`);
              _push2(ssrRenderComponent(unref(Link), {
                href: _ctx.route("admin.resource-sessions.create"),
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId2}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"${_scopeId2}></path></svg> Create Resource Session `);
                  } else {
                    return [
                      (openBlock(), createBlock("svg", {
                        class: "w-4 h-4",
                      ...
                      createTextVNode(" Create Resource Session ")
                    ];
                  }
                }),
                _: 1
            ...
            if (__props.sessions.links && __props.sessions.links.length > 3) {
              ...
              ssrRenderList(__props.sessions.links, (link) => {
                ...
                if (link.url) {
                  _push2(ssrRenderComponent(unref(Link), {
                    href: link.url,
                    class: `px-3 py-2 text-sm rounded-md transition-colors ${link.active ? "bg-orange-600 text-white" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`
                  }, {
                    default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                      if (_push3) {
                    ...
                    createVNode("div", { class: "p-6 bg-white border-b border-gray-200" }, [
                      createVNode("div", { class: "flex items-center justify-between mb-6" }, [
                        createVNode("div", null, [
                          createVNode("h2", { class: "text-2xl font-bold text-gray-900" }, " Resource Sessions "),
                          createVNode("p", { class: "text-gray-600 mt-1" }, " Manage resource sessions across multiple locations ")
                        ]),
                      ...
                      createVNode("div", { class: "mb-6" }, [
                        withDirectives(createVNode("input", {
                          ...
                          type: "text",
                          placeholder: "Search by location, city, country, or resource session name...",
                      ...
                      createVNode("div", { class: "overflow-x-auto" }, [
                        createVNode("table", { class: "min-w-full divide-y divide-gray-200" }, [
                          createVNode("thead", { class: "bg-gray-50" }, [
                            createVNode("tr", null, [
                              createVNode("th", { class: "px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" }, "Resource Program"),
                              ...
                              return openBlock(), createBlock("tr", {
                                ...
                                createVNode(unref(Link), {
                                  ...
                                  default: withCtx(() => {
                                    ...
                                    return [
                                      createVNode("td", { class: "px-6 py-4" }, [
                                        createVNode("div", { class: "text-sm font-medium text-gray-900" }, "📚 " + toDisplayString((_a2 = session.resource_program) == null ? void 0 : _a2.title), 1),
                                        createVNode("div", { class: "text-sm text-gray-500" }, toDisplayString((_b2 = session.resource_program) == null ? void 0 : _b2.slug), 1),
                                        createVNode("div", { class: "text-xs text-gray-600 mt-1" }, "📅 " + toDisplayString(formatDateRange(session)), 1)
                                      ])
                                    ];
                                  }),
                                ...
                                createVNode("td", { class: "px-6 py-4" }, [
                                  createVNode("div", { class: "text-sm font-medium text-gray-900" }, "⛰️ " + toDisplayString(session.location), 1),
                                ...
                                createVNode("td", { class: "px-6 py-4 whitespace-nowrap text-right text-sm font-medium" }, [
                                  createVNode("div", { class: "flex items-center justify-end space-x-2" }, [
                                    createVNode("button", {
                                      onClick: ($event) => togglePublish(session.id, session.is_active),
                                      class: [session.is_active ? "text-orange-600 hover:text-orange-800 bg-orange-50 hover:bg-orange-100" : "text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100", "px-3 py-1 rounded transition-colors text-sm font-medium"],
                      ...
                      !((_b = __props.sessions.data) == null ? void 0 : _b.length) ? (openBlock(), createBlock("div", {
                        key: 0,
                        class: "text-center py-12"
                      }, [
                        createVNode("div", { class: "text-gray-400 text-6xl mb-4" }, "📚"),
                        createVNode("h3", { class: "text-lg font-medium text-gray-900 mb-2" }, "No resource sessions found"),
                        createVNode("p", { class: "text-gray-500 mb-6" }, "Get started by creating your first resource session."),
                        createVNode(unref(Link), {
                          href: _ctx.route("admin.resource-sessions.create"),
                          class: "inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-medium rounded-lg shadow-sm hover:shadow-md transition-all duration-200"
                        }, {
                      ...
                      __props.sessions.links && __props.sessions.links.length > 3 ? (openBlock(), createBlock("div", {
                        key: 1,
                        class: "mt-6"
                      }, [
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/ResourceSessions/Index.vue");
