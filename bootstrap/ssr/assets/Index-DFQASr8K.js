import { ref, watch, computed, unref, createVNode, resolveDynamicComponent, withCtx, createBlock, createCommentVNode, openBlock, toDisplayString, withDirectives, vModelText, Fragment, renderList, vModelSelect, createTextVNode, useSSRContext } from "vue";
import { ssrRenderComponent, ssrRenderVNode, ssrRenderClass, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrRenderList, ssrLooseContain, ssrLooseEqual } from "vue/server-renderer";
  ...
  setup(__props) {
    ...
    const checkIn = (registrationId, type) => {
      if (confirm("Mark this registration as checked in?")) {
        const routeName = type === "workshops" ? "admin.registrations.workshops.check-in" : "admin.registrations.training-camps.check-in";
        router.post(
          route(routeName, registrationId),
          {},
          {
            preserveScroll: true
          }
    ...
    const uncheckIn = (registrationId, type) => {
      if (confirm("Uncheck this registration?")) {
        const routeName = type === "workshops" ? "admin.registrations.workshops.uncheck-in" : "admin.registrations.training-camps.uncheck-in";
        router.post(
          route(routeName, registrationId),
          {},
          {
    ...
    const downloadExport = () => {
      ...
      const exportRoute = currentTab.value === "workshops" ? route("admin.registrations.workshops.export") : route("admin.registrations.training-camps.export");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderVNode(_push, createVNode(resolveDynamicComponent(unref(Layout)), null, {
        header: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex justify-between items-center"${_scopeId}><div${_scopeId}><h2 class="text-xl font-semibold leading-tight text-gray-800"${_scopeId}> Registrations </h2>`);
            if (__props.isSchoolUser && __props.schoolName) {
              _push2(`<p class="text-sm text-gray-500 mt-1"${_scopeId}> Showing students from: <span class="font-medium"${_scopeId}>${ssrInterpolate(__props.schoolName)}</span></p>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div class="flex items-center gap-2"${_scopeId}><button class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md"${_scopeId}> Export CSV </button><button class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-md"${_scopeId}> Export Excel </button></div></div>`);
          } else {
            return [
              createVNode("div", { class: "flex justify-between items-center" }, [
                createVNode("div", null, [
                  createVNode("h2", { class: "text-xl font-semibold leading-tight text-gray-800" }, " Registrations "),
                  __props.isSchoolUser && __props.schoolName ? (openBlock(), createBlock("p", {
                    key: 0,
                    class: "text-sm text-gray-500 mt-1"
                  }, [
                    createTextVNode(" Showing students from: "),
                    createVNode("span", { class: "font-medium" }, toDisplayString(__props.schoolName), 1)
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (showExportModal.value) {
              _push2(`<div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true"${_scopeId}><div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0"${_scopeId}><div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"${_scopeId}></div><div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"${_scopeId}><div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4"${_scopeId}><div class="sm:flex sm:items-start"${_scopeId}><div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-100 sm:mx-0 sm:h-10 sm:w-10"${_scopeId}><svg class="h-6 w-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"${_scopeId}></path></svg></div><div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full"${_scopeId}><h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title"${_scopeId}> Export ${ssrInterpolate(currentTab.value === "workshops" ? "Workshop" : "Training Camp")} Registrations </h3><div class="mt-4 space-y-4"${_scopeId}><p class="text-sm text-gray-500"${_scopeId}> Select filters to export registration records as CSV or Excel </p><div${_scopeId}><label for="export-status" class="block text-sm font-medium text-gray-700"${_scopeId}>Check-in Status</label><select id="export-status" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(exportFilters.value.status) ? ssrLooseContain(exportFilters.value.status, "") : ssrLooseEqual(exportFilters.value.status, "")) ? " selected" : ""}${_scopeId}> All Statuses </option><option value="checked_in"${ssrIncludeBooleanAttr(Array.isArray(exportFilters.value.status) ? ssrLooseContain(exportFilters.value.status, "checked_in") : ssrLooseEqual(exportFilters.value.status, "checked_in")) ? " selected" : ""}${_scopeId}> Checked In </option><option value="pending"${ssrIncludeBooleanAttr(Array.isArray(exportFilters.value.status) ? ssrLooseContain(exportFilters.value.status, "pending") : ssrLooseEqual(exportFilters.value.status, "pending")) ? " selected" : ""}${_scopeId}> Pending Check-in </option></select></div><div${_scopeId}><label for="export-session" class="block text-sm font-medium text-gray-700"${_scopeId}>${ssrInterpolate(currentTab.value === "workshops" ? "Workshop" : "Training Camp")} Session </label><select id="export-session" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(exportFilters.value.session_id) ? ssrLooseContain(exportFilters.value.session_id, "") : ssrLooseEqual(exportFilters.value.session_id, "")) ? " selected" : ""}${_scopeId}> All Sessions </option><!--[-->`);
              ssrRenderList(__props.sessions, (session) => {
                _push2(`<option${ssrRenderAttr("value", session.id)}${ssrIncludeBooleanAttr(Array.isArray(exportFilters.value.session_id) ? ssrLooseContain(exportFilters.value.session_id, session.id) : ssrLooseEqual(exportFilters.value.session_id, session.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(session.title)}</option>`);
              });
              _push2(`<!--]--></select></div></div></div></div></div><div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse"${_scopeId}><button type="button" class="${ssrRenderClass(
                exportFormat.value === "excel" ? "w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm" : "w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm"
              )}"${_scopeId}>${ssrInterpolate(exportFormat.value === "excel" ? "Download Excel" : "Download CSV")}</button><button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"${_scopeId}> Cancel </button></div></div></div></div>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
                ...
                createVNode("div", { class: "max-w-screen-2xl mx-auto sm:px-6 lg:px-8" }, [
                  createVNode("div", { class: "mb-8" }, [
                    createVNode("nav", {
                      ...
                      createVNode("button", {
                        onClick: ($event) => switchTab("workshops"),
                        class: [
                          currentTab.value === "workshops" ? "border-indigo-500 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300",
                          "whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200"
                        ]
                      }, " 🎓 Workshop Registrations ", 10, ["onClick"]),
                      createVNode("button", {
                        onClick: ($event) => switchTab("training-camps"),
                        class: [
                          currentTab.value === "training-camps" ? "border-orange-500 text-orange-600" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300",
                          "whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200"
                        ]
                      }, " 🏕️ Training Camp Registrations ", 10, ["onClick"])
                    ])
                  ]),
                  ...
                  createVNode("div", { class: "bg-white shadow rounded-lg mb-8" }, [
                            ...
                            return openBlock(), createBlock("tr", {
                              ...
                              createVNode("td", { class: "px-6 py-4" }, [
                                createVNode("div", { class: "text-sm text-gray-900" }, [
                                  createVNode("div", { class: "font-medium" }, toDisplayString(getSessionInfo(registration).location), 1),
                                  getSessionInfo(registration).date ? (openBlock(), createBlock("div", {
                                    key: 0,
                                    class: "text-xs text-gray-500"
                                  }, toDisplayString(getSessionInfo(registration).date), 1)) : createCommentVNode("", true),
                                  getSessionInfo(registration).time ? (openBlock(), createBlock("div", {
                                    key: 1,
                                    class: "text-xs text-gray-500"
                                  }, toDisplayString(getSessionInfo(registration).time), 1)) : createCommentVNode("", true)
                                ])
                              ]),
                              createVNode("td", { class: "px-6 py-4 whitespace-nowrap" }, [
                                createVNode("span", {
                              ...
                              createVNode("td", { class: "px-6 py-4 whitespace-nowrap text-sm text-gray-500" }, toDisplayString(formatDateTime(
                                registration.created_at
                              )), 1),
                      ...
                      createVNode("div", { class: "sm:flex sm:items-start" }, [
                        ...
                        createVNode("div", { class: "mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full" }, [
                          createVNode("h3", {
                            ...
                            id: "modal-title"
                          }, " Export " + toDisplayString(currentTab.value === "workshops" ? "Workshop" : "Training Camp") + " Registrations ", 1),
                          createVNode("div", { class: "mt-4 space-y-4" }, [
                            ...
                            createVNode("div", null, [
                              createVNode("label", {
                                for: "export-session",
                                class: "block text-sm font-medium text-gray-700"
                              }, toDisplayString(currentTab.value === "workshops" ? "Workshop" : "Training Camp") + " Session ", 1),
                              withDirectives(createVNode("select", {
                                id: "export-session",
                                "onUpdate:modelValue": ($event) => exportFilters.value.session_id = $event,
                                class: "mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"
                              }, [
                                createVNode("option", { value: "" }, " All Sessions "),
                                (openBlock(true), createBlock(Fragment, null, renderList(__props.sessions, (session) => {
                                  return openBlock(), createBlock("option", {
                                    key: session.id,
                                    value: session.id
                                  }, toDisplayString(session.title), 9, ["value"]);
                                }), 128))
                              ], 8, ["onUpdate:modelValue"]), [
                                [vModelSelect, exportFilters.value.session_id]
                              ])
                            ])
                          ])
                        ])
